...
VideoOutputPipeline::VideoOutputPipeline(VideoOutputTrackInfo info)
  ...
  switch (info_.codec) {
    case VideoCodec::H264: {
      ...
      g_object_set(payload_, "config-interval", 1, NULL);
  ...
  g_object_set(sink_, "host", info_.destination_host.c_str(), NULL);
  g_object_set(sink_, "port", info_.destination_port, NULL);
  g_object_set(sink_, "sync", false, NULL);
  g_object_set(sink_, "async", false, NULL);
